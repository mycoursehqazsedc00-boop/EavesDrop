...
function EavesDrop:OnInitialize()
	...
	self:RegisterDB("EavesDropDB","EavesDropStatsDB")
	self:RegisterDefaults('profile', default_config )
	self:RegisterDefaults('char', {Stats = {[OUTGOING] = {},[INCOMING] = {}}})
...
function EavesDrop:CombatEvent(info)
    ...
    local honor = tonumber(text) or tonumber(info.amount) or 0
    self:DisplayEvent(MISC, "+"..honor.." ("..HONOR..")", texture, self.db.profile["HONORC"])
	end
end
...
function EavesDrop:DisplayEvent(type, text, texture, color)
	...
	if pEvent.text == L["StartCombat"] then
		if pEvent.tooltipText ~= nil then
			pEvent.tooltipText = pEvent.tooltipText..L["StartCombatTooltip"]
		else
			pEvent.tooltipText = L["StartCombatTooltip"]
		end
	end
	if pEvent.text == L["EndCombat"] then
		if pEvent.tooltipText ~= nil then
			pEvent.tooltipText = pEvent.tooltipText..L["EndCombatTooltip"]
		else
			pEvent.tooltipText = L["EndCombatTooltip"]
		end
	end
	if pEvent.text == deathchar..UnitName("player")..deathchar then
		if pEvent.tooltipText ~= nil then
			pEvent.tooltipText = pEvent.tooltipText..L["DeathTooltip"]
		else
			pEvent.tooltipText = L["DeathTooltip"]
		end
	end
...
function EavesDrop:SendLineToChat(text)
...
function EavesDrop:SendHistoryToChat(text, part)
	local nocrit = false
	local nonormal = false
	local teststring = string.gsub(text, "\|", "")
	local firstlinestart = string.find(teststring, "cffffffff", 1)
	local secondlinestart = string.find(teststring, "cffffffff", 3)
	local channel, chatnumber = ChatFrameEditBox.chatType
	if channel == "WHISPER" then
		chatnumber = ChatFrameEditBox.tellTarget
	elseif channel == "CHANNEL" then
	...
	if secondlinestart == nil then
		nocrit = true
	elseif secondlinestart == 3 then
		nonormal = true
		secondlinetime = string.sub(teststring, secondlinestart+9, secondlinestart+25)
		secondlinetext = string.sub(teststring, secondlinestart+28)
	else
		secondlinetime = string.sub(teststring, secondlinestart+9, secondlinestart+25)
		secondlinetext = string.sub(teststring, secondlinestart+28)
	end
	if firstlinestart == 1 and secondlinestart == nil then
		firstlinetime = string.sub(teststring, firstlinestart+9, firstlinestart+25)
		firstlinetext = string.sub(teststring, firstlinestart+28)
	elseif firstlinestart == 1 and secondlinestart ~= nil then
		firstlinetime = string.sub(teststring, firstlinestart+9, firstlinestart+25)
		firstlinetext = string.sub(teststring, firstlinestart+28, secondlinestart-3)
	end
	if part == 1 and not nonormal then
		SendChatMessage(firstlinetime.." - "..firstlinetext, channel, nil, chatnumber);
	elseif part == 2 and not nocrit then
		SendChatMessage(secondlinetime.." - "..secondlinetext, channel, nil, chatnumber);
	end
end
